...
def mask_tras_dets(cls_num, det_labels, tra_labels) -> np.array:
    ...
    cls_mask = (
        np.ones(shape=(cls_num, det_num, tra_num)) * np.arange(cls_num)[:, None, None]
    )
...
def reorder_metrics(metrics: dict) -> dict:
    ...
    for cls, metric in metrics.items():
        if metric in new_metrics:
            new_metrics[metric].append(cls)
        else:
            new_metrics[metric] = [cls]
    return new_metrics


def spec_metric_mask(
    cls_list: list, det_labels: np.array, tra_labels: np.array
) -> np.array:
    """
