...
Overlap metrics used by the CANMOT paper tracker.
half-parallel: iou_bev, iou_3d, giou_bev, giou_3d
Serial: iou_bev_s, iou_3d_s, giou_bev_s, giou_3d_s
...
def iou_bev_s(box_a: NuscBox, box_b: NuscBox) -> float:
    ...
    if box_b.name != box_a.name:
        return -np.inf
    boxa_corners, boxb_corners = np.array(box_a.bottom_corners_), np.array(
        box_b.bottom_corners_
    )
...
def iou_3d_s(box_a: NuscBox, box_b: NuscBox) -> Tuple[float, float]:
    ...
    overlap_height = max(
        0, min(za + ha / 2, zb + hb / 2) - max(za - ha / 2, zb - hb / 2)
    )
...
def giou_3d_s(box_a: NuscBox, box_b: NuscBox) -> Tuple[float, float]:
    ...
    giou3d = (
        inter_volume / union_volume - (convex_volume - union_volume) / convex_volume
    )
    gioubev = inter_area / union_area - (convex_area - union_area) / convex_area
...
def giou_3d(boxes_a: dict, boxes_b: dict) -> Tuple[np.array, np.array]:
    ...
    bcs_a, bcs_b = (
        boxes_a["np_dets_bottom_corners"],
        boxes_b["np_dets_bottom_corners"],
    )  # [box_num, 4, 2]
    ...
    rep_bcs_a, rep_bcs_b = expand_dims(bcs_a, len(bcs_b), 1), expand_dims(
        bcs_b, len(bcs_a), 0
    )  # [a_num, b_num, 4, 2]
    wlh_a, wlh_b = expand_dims(infos_a[:, 3:6], len(infos_b), 1), expand_dims(
        infos_b[:, 3:6], len(infos_a), 0
    )
    za, zb = expand_dims(infos_a[:, 2], len(infos_b), 1), expand_dims(
        infos_b[:, 2], len(infos_a), 0
    )  # [a_num, b_num]
    ...
    ohs = np.maximum(
        np.zeros_like(ha),
        np.minimum(za + ha / 2, zb + hb / 2) - np.maximum(za - ha / 2, zb - hb / 2),
    )
    uhs = np.maximum((za + ha / 2), (zb + hb / 2)) - np.minimum(
        (zb - hb / 2), (za - ha / 2)
    )
    ...
    union_areas, union_volumes = (
        wa * la + wb * lb - inter_areas,
        wa * la * ha + wb * lb * hb - inter_volumes,
    )
    ...
    giou3d = (
        ...
        - (convex_volumes - union_volumes) / convex_volumes
    )
...
def quaternion_to_yaw(w, x, y, z):
    """
    Calculates the yaw angle (rotation around the z-axis) from a quaternion.
    Args:
