...
def PolyArea2D(pts: np.array) -> np.array:
    ...
    area = (
        np.abs(
            np.sum(
                (pts[:, :, 0] * roll_pts[:, :, 1] - pts[:, :, 1] * roll_pts[:, :, 0]),
                axis=1,
            )
        )
        * 0.5
...
def mask_between_boxes(
    labels_a: np.array, labels_b: np.array
...
def logical_or_mask(
    mask: np.array, seq_mask: np.array, boxes_a: dict, boxes_b: dict
) -> np.array:
    """
...
def loop_inter(
    polys1: List[Polygon], polys2: List[Polygon], mask: np.array
) -> np.array:
    """
...
def loop_convex(
    bottom_corners_a: np.array, bottom_corners_b: np.array, mask: np.array
) -> np.array:
    """
    ...
    def init_convex(bcs: np.array, mask_: np.array) -> np.array:
        ...
        return [
            ConvexHull(bc) if not mask_[i] else fake_convex for i, bc in enumerate(bcs)
        ]

    all_bcs = np.concatenate((bottom_corners_a, bottom_corners_b), axis=2).reshape(
        -1, 8, 2
    )  # [numa * numb, 8, 2]
    ...
    conv_nums = (
        np.array([len(conv_cor) for conv_cor in conv_cors])
        .reshape(1, -1)
        .repeat(9, axis=0)
    )  # [9, numa * numb]
    ...
    for conv_num, valid_idx in enumerate(row_valid_idx):
        if len(valid_idx[0]) == 0:
            continue
        b_idx = (
            np.arange(len(valid_idx[0])).reshape(-1, 1).repeat(conv_num, axis=1)
        )  # [len(valid_idx), conv_num]
        i_idx = np.stack(
            [np.array(cor, dtype=int) for cor in conv_cors[valid_idx]]
        )  # [len(valid_idx), conv_num]
