...
from canmot.motion_module.kalman_filter import (
    LinearKalmanFilter,
    ExtendKalmanFilter,
    ExtendedKalmanFilterRotate,
)
from canmot.data.script.NUSC_CONSTANT import CLASS_STR_TO_SEG_CLASS

logger = logging.getLogger(__name__)


class Trajectory:
    logged_class_names = set()
    def __init__(self, timestamp: int, config: dict, track_id: int, det_infos: dict):
        # init basic infos
        self.timestamp = timestamp
        self.cfg, self.tracking_id, self.class_label = (
            config,
            track_id,
            det_infos["np_array"][-1],
        )
        ...
        self.score_management = ScoreManagement(
            timestamp, config, self.class_label, det_infos
        )
        # manage for tracklet's motion/geometric infos
        KF_type = self.cfg["motion_model"]["filter"][self.class_label]
        model_name = self.cfg["motion_model"]["model"][self.class_label]
        class_name = CLASS_STR_TO_SEG_CLASS.get(int(self.class_label), f"cls{self.class_label}")
        assert KF_type in [
            "LinearKalmanFilter",
            "ExtendKalmanFilter",
            "ExtendedKalmanFilterRotate",
        ], "must use specific kalman filter"

        if class_name not in self.logged_class_names:
            logger.info(
                "Initialized %s with model %s for class %s (id=%d, track_id=%d)",
                KF_type, model_name, class_name, int(self.class_label), track_id,
            )
            self.logged_class_names.add(class_name)

        self.motion_management = globals()[KF_type](
            timestamp, config, track_id, det_infos
        )
