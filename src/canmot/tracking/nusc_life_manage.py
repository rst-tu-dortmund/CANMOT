...
class LifeManagement:
    def __init__(self, timestamp: int, config: dict, class_label: int):
        ...
        self.min_hit, self.max_age = (
            self.cfg["min_hit"][class_label],
            self.cfg["max_age"][class_label],
        )
        self.state = (
            "active" if self.min_hit <= 1 or timestamp <= self.min_hit else "tentative"
        )
    ...
    def update(self, timestamp: int, det=None) -> None:
        ...
        if self.state == "tentative":
            ...
            raise Exception("dead trajectory cannot be updated")

    def __repr__(self) -> str:
        repr_str = "init_timestamp: {}, time_since_update: {}, hit: {}, state: {}"
        return repr_str.format(
            self.init_time, self.time_since_update, self.hit, self.state
        )
