...
class ScoreObject:
    ...
    def __repr__(self) -> str:
        repr_str = (
            "Raw score: {}, Predict score: {}, Update score: {}, Final score: {}."
        )
        return repr_str.format(
            self.raw_score, self.predict_score, self.update_score, self.final_score
        )


class ScoreManagement:
    def __init__(
        self, timestamp: int, cfg: dict, cls_label: int, det_infos: dict
    ) -> None:
    ...
    def predict(self, timestamp: int, pred_obj: FrameObject = None) -> None:
        ...
        pred_obj.predict_box.score = pred_obj.predict_infos[-5] = (
            score_obj.predict_score
        )

    def update(
        self, timestamp: int, update_obj: FrameObject, raw_det: dict = None
    ) -> None:
        ...
        update_obj.update_box.score = update_obj.update_infos[-5] = raw_det[
            "nusc_box"
        ].score
