...
import os
import numpy as np
from canmot.pre_processing.nusc_nms import blend_nms
...
from canmot.utils.script import (
    mask_tras_dets,
    fast_compute_check,
    reorder_metrics,
    spec_metric_mask,
)
from canmot.utils.figure2_viz import render_prediction_snapshot
...
class Tracker:
    def __init__(self, config):
        ...
        # Log resolved motion_model config for verification
        _tracker_logger = self.logger
        _tracker_logger.info("=" * 70)
        _tracker_logger.info("TRACKER CONFIG: motion_model.filter = %s", config["motion_model"]["filter"])
        _tracker_logger.info("TRACKER CONFIG: motion_model.model  = %s", config["motion_model"]["model"])
        if "limit_classes" in config:
            _tracker_logger.info("TRACKER CONFIG: limit_classes = %s", config["limit_classes"])
        _tracker_logger.info("=" * 70)
        # Hyper parameters
        self.is_debug = self.cfg["debug"]["is_debug"]
        self.cls_num = self.cfg["basic"]["CLASS_NUM"]
        self.f_thre, self.s_thre = (
            config["association"]["first_thre"],
            config["association"]["second_thre"],
        )
        self.two_stage, self.algorithm = (
            config["association"]["two_stage"],
            config["association"]["algorithm"],
        )

        self.punish_num, self.metrics = (
            config["output"]["punish_num"],
            config["association"]["category_metrics"],
        )
        ...
        self.id_seed, self.frame_id, self.seq_id, self.det_infos, self.tra_infos = (
            0,
            None,
            None,
            None,
        ...
        self.viz_cfg = self.cfg.get("visualization", {}) if isinstance(self.cfg, dict) else {}
        self.viz_enabled = bool(self.viz_cfg.get("enable", False))
        self.viz_every_n_steps = int(self.viz_cfg.get("every_n_steps", 35))
        if self.viz_every_n_steps <= 0:
            self.viz_every_n_steps = 35
        self.predict_step_counter = 0
        base_output = (
            self.cfg.get("result_path")
            or self.cfg.get("eval_path")
            or self.cfg.get("environment_cfg", {}).get("logging_path")
            or "outputs"
        )
        if not os.path.isabs(base_output):
            base_output = os.path.abspath(base_output)
        viz_subdir = self.viz_cfg.get("output_subdir", "tracking_visualizations")
        self.viz_output_dir = os.path.join(base_output, viz_subdir)
        if self.viz_enabled:
            os.makedirs(self.viz_output_dir, exist_ok=True)
            self.logger.info("Visualization output directory: %s", self.viz_output_dir)
    ...
    def tracking(self, data_info: dict) -> None:
        ...
        if data_info["is_first_frame"]:
            self.reset()
        self.det_infos, self.frame_id, self.seq_id = (
            data_info,
            data_info["frame_id"],
            data_info["seq_id"],
        )
        ...
        if self.det_infos["no_dets"]:
            ...
            if self.post_nms_cfg["post_nms"]:
                self.post_nms_tras(data_info)
            return
        ...
        # only for debug
        assert (
            len(self.tentative_tras) == 0
        ), "no tentative tracklet in the best performance version."
    ...
    def tras_predict(self) -> None:
        ...
        if self.is_debug:
            self.debug()
        self.predict_step_counter += 1
        pre_infos, pre_states_meas_cov = [], []
        pred_infos, pred_bms, pred_boxes, all_valid_ids = [], [], [], []
        states_meas, innovation_information = [], []
        states_meas_cov, measurement_noise_cov = [], []
        ...
        for tra_id, tra in self.valid_tras.items():
            ...
            motion = tra.motion_management
            pre_state = np.array(motion.state, copy=True)
            pre_state = motion.model.warpStateYawToPi(pre_state)
            pre_info = np.array(motion.getOutputInfo(pre_state), copy=True)
            if pre_info.shape[0] > 12:
                try:
                    pre_score = float(tra.score_management[self.frame_id - 1].final_score)
                except Exception:
                    pre_score = float(pre_info[12])
                pre_info[12] = pre_score
            pre_infos.append(pre_info)
            pre_states_meas_cov.append(motion.get_covariance_meas())
            ...
            _pred_infos = np.array(pred_object.predict_infos, copy=True)
            _pred_infos[12] = pred_object.predict_box.score  # Update score in predict_infos
            pred_infos.append(_pred_infos)
            pred_bms.append(pred_object.predict_bms)
            states_meas.append(tra.motion_management.state_meas_space.squeeze(-1))
            innovation_information.append(tra.motion_management.innovation_information)
            states_meas_cov.append(tra.motion_management.get_covariance_meas())
            measurement_noise_cov.append(
                tra.motion_management.get_measurement_noise_covariance()
            )
        ...
        self.tra_infos = {
            "np_tras": np.array(
                pred_infos
            ),  # info dim: 17, add 'tracking_id', 'seq_id', 'frame_id'
            "np_tras_bottom_corners": np.array(pred_bms),
            "all_valid_ids": np.array(all_valid_ids),
            "all_valid_boxes": np.array(pred_boxes),
            "tra_num": len(all_valid_ids),
            "states_meas": np.array(states_meas),
            "innovation_information": np.array(innovation_information),
            "states_meas_cov": np.array(states_meas_cov),
            "measurement_noise_cov": np.array(measurement_noise_cov),
            "pre_np_tras": np.array(pre_infos),
            "pre_states_meas_cov": np.array(pre_states_meas_cov),
        }

        if (
            self.viz_enabled
            and len(all_valid_ids) > 0
            and self.predict_step_counter % self.viz_every_n_steps == 0
            and (
                self.viz_cfg.get("sequence_ids") is None
                or self.seq_id in self.viz_cfg.get("sequence_ids")
            )
        ):
            try:
                self.logger.info(
                    "Rendering prediction snapshot for seq=%s frame=%s at predict step %s",
                    self.seq_id,
                    self.frame_id,
                    self.predict_step_counter,
                )
                render_prediction_snapshot(
                    base_infos=self.tra_infos["pre_np_tras"],
                    base_states_meas_cov=self.tra_infos["pre_states_meas_cov"],
                    all_valid_ids=self.tra_infos["all_valid_ids"],
                    frame_id=self.frame_id,
                    seq_id=self.seq_id,
                    predict_step=self.predict_step_counter,
                    dt=self.cfg["basic"]["LiDAR_interval"],
                    output_dir=self.viz_output_dir,
                    logger=self.logger,
                )
            except Exception as err:
                self.logger.warning(
                    "Prediction visualization failed at seq=%s frame=%s: %s",
                    self.seq_id,
                    self.frame_id,
                    err,
                )
    ...
    def tras_punish(self, data_info: dict) -> None:
        ...
        # manage trajectory
        dict_track_res = self.tras_update(
            tracking_ids=np.array([]), data_info=data_info
        )
        if len(dict_track_res["np_track_res"]) == 0:
            data_info.update({"no_val_track_result": True})
            return
    ...
    def tras_update(self, tracking_ids: np.array, data_info: dict) -> dict:
        ...
        for tra_id, tra in tmp_merge_tras.items():
            ...
            if tra.life_management.state == "active":
                ...
                raise Exception("Tracjectory state only have three attributes")
    ...
    def data_association(self) -> np.array:
        ...
        if len(self.valid_tras) == 0:
            ids = np.arange(
                self.id_seed, self.id_seed + self.det_infos["det_num"], dtype=int
            )
    ...
    def compute_cost(self) -> dict:
        ...
        det_labels, tra_labels = (
            self.det_infos["np_dets"][:, -1],
            self.tra_infos["np_tras"][:, -4],
        )
        ...
        tra_cost_infos = {
            "np_dets": self.tra_infos["np_tras"][:, :-3],
            "np_dets_bottom_corners": self.tra_infos["np_tras_bottom_corners"],
            "states_meas": self.tra_infos["states_meas"],
            "innovation_information": self.tra_infos["innovation_information"],
            "states_meas_cov": self.tra_infos["states_meas_cov"],
            "measurement_noise_cov": self.tra_infos["measurement_noise_cov"],
        }
        ...
        if self.fast:
            ...
            for metric, cls_list in self.re_metrics.items():
                ...
                tra_cost_infos["mask"] = spec_metric_mask(
                    cls_list, det_labels, tra_labels
                )
                if metric in METRIC:
    ...
    def matching_cost(self, cost_matrices: dict) -> np.array:
        ...
        m_dets_1, m_tras_1, um_dets_1, um_tras_1 = globals()[self.algorithm](
            cost1, self.f_thre
        )
        if self.two_stage:
        ...
        assert len(m_dets_1) == len(
            m_tras_1
        ), "as the pair, number of the matched tras and dets must be equal"
        ...
        if len(m_dets_1) == 0:
            ...
            ids, match_pairs = [], {
                key: value for key, value in zip(m_dets_1, m_tras_1)
            }
            all_valid_ids = self.tra_infos["all_valid_ids"]
    ...
    def post_nms_tras(self, data_info) -> None:
        ...
        """
        if "no_val_track_result" in data_info:
            return
        post_metric = self.post_nms_cfg["NMS_metric"]
        post_thre = self.post_nms_cfg["NMS_thre"]
        post_type = self.post_nms_cfg["NMS_type"]
        tmp_tra_infos = {
            "np_dets": np.array(data_info["np_track_res"])[:, :-3],
            "np_dets_bottom_corners": np.array(data_info["bm_track_res"]),
        }
        keep = globals()[post_type](
            box_infos=tmp_tra_infos, metrics=post_metric, thre=post_thre
        )
        if len(keep) == 0:
            data_info.update({"no_val_track_result": True})
        else:
            data_info["np_track_res"] = np.array(data_info["np_track_res"])[
                keep
            ].tolist()
            data_info["box_track_res"] = np.array(data_info["box_track_res"])[
                keep
            ].tolist()

    def debug(self) -> None:
        """Check that trajectory lifecycle sets do not overlap."""
