...
def concat_box_attr(nuscbox: NuscBox, *attrs) -> List:
    ...
    for attr in attrs:
        ...
        if isinstance(tmp_attr, list):
            ...
            raise Exception("unsupport date format to concat")
    return res
...
def dictdet2array(dets: List[dict], *attrs) -> Tuple[List, np.array]:
    listdets = [
        concat_dict_attr(det, *attrs)
        for det in dets
        if det["detection_name"] in CLASS_SEG_TO_STR_CLASS
    ]
...
def arraydet2box(dets: np.array, ids: np.array = None):
    ...
    for idx, det in enumerate(dets):
        curr_box = NuscBox(
            ...
            size=det[3:6],
            rotation=det[8:12],
            velocity=tuple(det[6:8].tolist() + [0.0]),
            score=det[12],
            name=CLASS_STR_TO_SEG_CLASS[int(det[13])],
        )
        if ids is not None:
            curr_box.tracking_id = int(ids[idx])
